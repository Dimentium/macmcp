...
final class PackagingScriptTests: XCTestCase {
    ...
    func testLocalInstallScriptPinsSidecarsAndDoesNotResetTCC() throws {
        ...
        XCTAssertTrue(script.contains("existing ChatGPT tunnel configuration is unreadable"))
        XCTAssertTrue(script.contains("chatgpt_tunnel_client=\"$(sed -n '2p' <<< \"$existing_tunnel_fields\")\""))
        XCTAssertTrue(script.contains("payload[\"chatGPTTunnel\"][\"clientPath\"] = tunnel_client"))
        XCTAssertTrue(script.contains("pgrep -f -x \"$chatgpt_tunnel_client run --profile $chatgpt_tunnel_profile\""))
        XCTAssertFalse(script.contains("terminate_matching_command \"tunnel-client run --profile $chatgpt_tunnel_profile\""))
    ...
    func testPinnedEventKitBuildScriptVerifiesTheSourceAndDependencyGraph() throws {
        ...
        XCTAssertTrue(script.contains("retry_git_network()"))
        XCTAssertTrue(script.contains("else\n      exit_code=$?"))
        XCTAssertTrue(script.contains("retry_git_network \"CheICalMCP clone\" git clone"))
        XCTAssertTrue(script.contains("retry_git_network \"CheICalMCP fetch\" git -C"))
    ...
    func testPinnedMailBuildScriptVerifiesTheSourceAndGoDependencyGraph() throws {
        ...
        XCTAssertTrue(script.contains("retry_git_network()"))
        XCTAssertTrue(script.contains("else\n      exit_code=$?"))
        XCTAssertTrue(script.contains("retry_git_network \"mail-mcp clone\" git clone"))
        XCTAssertTrue(script.contains("retry_git_network \"mail-mcp fetch\" git -C"))
    ...
    func testReleaseScriptPublishesTheFullSignedCaskFlow() throws {
        ...
        XCTAssertTrue(script.contains("retry_local_app_launch"))
        XCTAssertTrue(script.contains("MacMCP was not launched by LaunchServices"))
        XCTAssertTrue(script.contains("open /Applications/MacMCP.app"))
    ...
    func testPackagedVersionsMatchTheNextSourceRelease() throws {
        ...
        XCTAssertTrue(appVersion.contains("static let version = \"0.2.31\""))
        XCTAssertTrue(appInfo.contains("<string>0.2.31</string>"))
        XCTAssertEqual(bundleInfo.components(separatedBy: "<string>0.2.31</string>").count, 3)
    }
    ...
    func testDeploymentAcceptanceScriptIsIsolated() throws {
        ...
        XCTAssertTrue(script.contains("--reuse-existing-configuration"))
        XCTAssertTrue(script.contains("prepare-tunnel-reuse"))
        XCTAssertTrue(script.contains("foreign-tunnel-client"))
        XCTAssertTrue(script.contains("kill -0 \"$foreign_tunnel_pid\""))
        XCTAssertTrue(script.contains("stop_foreign_tunnel"))
        XCTAssertTrue(script.contains("Contents/Resources/tunnel-client/tunnel-client"))
    ...
    func testLocalUninstallScriptPreservesKeychainByDefault() throws {
        ...
        XCTAssertTrue(script.contains("managed_tunnel_fields()"))
        XCTAssertTrue(script.contains("pgrep -f -x \"$tunnel_client_path run --profile $tunnel_profile\""))
        XCTAssertFalse(script.contains("terminate_matching_command \"tunnel-client run --profile $tunnel_profile\""))
        XCTAssertTrue(script.contains("wait_for_existing_runtime_exit 3"))
        XCTAssertTrue(script.contains("wait_for_existing_runtime_exit 2 ||"))
