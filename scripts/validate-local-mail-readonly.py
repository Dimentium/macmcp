...
def load_mcp_config(path: Path) -> tuple[str, list[str]]:
    # Cask installs keep the canonical MCP configuration in the app-owned
    # launch record. Use the app bridge directly when no standalone config was
    # generated by a source install.
    if not path.is_file():
        ...
        socket_path = Path(
            os.environ.get(
                "MACMCP_IPC_SOCKET",
                str(Path.home() / "Library/Application Support/macmcp/mcp.sock"),
            )
        ).expanduser()
        ...
        return command, ["--stdio-proxy", str(socket_path)]
    try:
