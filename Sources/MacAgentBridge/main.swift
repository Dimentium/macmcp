...
if CommandLine.arguments.contains("--verify-configured-mail-keychain") {
    do {
        ...
        let launchConfigurationURL = AppLaunchConfigurationStore.defaultFileURL()
        writeStderr("Configured mail Keychain check: reading launch configuration")
        let storedArguments = try AppLaunchConfigurationStore(
            fileURL: launchConfigurationURL
        ).readArguments() ?? []
        ...
        for account in configuration.mailAccounts {
            secrets.append(try KeychainCredentialStore().readSecret(account: account.username))
        }
...
if let index = userArguments.firstIndex(where: { storePasswordOptions.contains($0) }) {
    ...
    do {
        ...
        try KeychainCredentialStore().storeSecret(password, account: userArguments[index + 1])
...
if let index = userArguments.firstIndex(where: { deletePasswordOptions.contains($0) }) {
    ...
    do {
        try KeychainCredentialStore().deleteSecret(account: userArguments[index + 1])
...
do {
    ...
    writeStderr("mac-agent-bridge failed: \(error.localizedDescription)")
    exit(1)
}
