...
struct ManagedDraftKeyStore: Sendable {
    static let service = "com.dimentium.macmcp.managed-drafts"
    static let legacyService = "com.openai.macmcp.managed-drafts"
    ...
    init(
        credentials: any CredentialStore = MigratingCredentialStore(
            primary: KeychainCredentialStore(service: Self.service),
            legacy: KeychainCredentialStore(service: Self.legacyService)
        )
    ) {
